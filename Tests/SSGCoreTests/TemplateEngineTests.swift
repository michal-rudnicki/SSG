...
struct TemplateEngineTests {
    ...
    func multipleKeys() {
        let result = TemplateEngine.render(
            ...
            context: ["title": "Strona", "content": "Treść"]
        )
    ...
    func multipleOccurrencesOfSameKey() {
        let result = TemplateEngine.render(
            template: "{{site.title}} | {{site.title}}",
            context: ["site.title": "Mój Blog"]
        )
